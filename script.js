...
function initializing_left_panel()      // for initializing the left panel of icons and names
{
    for(let j=1;j<=5;j++)
    {
        ...
        if(details[j-1].title.length>31)                //  codition  to prevent overflow and make it at center
        {
            let total_length = details[j-1].title.length;
            dashed_title = details[j-1].title.substring(0,13) + ' ... ' + details[j-1].title.substring(total_length-13,total_length); 
